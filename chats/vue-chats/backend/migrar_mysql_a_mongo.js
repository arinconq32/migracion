...
function resolveDbValue(primary, fallback) {
...
const MYSQL_CONFIG = {
  host: resolveDbValue(process.env.MYSQL_HOST, process.env.DB_HOST) || "localhost",
  port: process.env.MYSQL_PORT ? parseInt(process.env.MYSQL_PORT) : 3306,
  user: resolveDbValue(process.env.MYSQL_USER, process.env.DB_USER) || "root",
  password: resolveDbValue(process.env.MYSQL_PASSWORD, process.env.DB_PASSWORD) || "",
  database: resolveDbValue(process.env.MYSQL_DB, process.env.DB_NAME) || "omnicanalidad",
};
...
async function main() {
  ...
  if (convs.length) {
    ...
    try {
      await conversacionesCol.insertMany(docs, { ordered: false });
      console.log(`Migradas ${docs.length} conversaciones.`);
    } catch (err) {
      const inserted = err?.result?.result?.nInserted ?? err?.insertedDocs?.length;
      console.warn(
        `Conversaciones con duplicados parciales. Insertadas: ${inserted ?? 0}/${docs.length}.`,
      );
    }
  }
