...
function normalizeMessage(msg) {
  ...
  let archivoUrl = String(
    msg.archivoUrl || msg.archivo_url || msg.mediaUrl || msg.media_url || msg.url || "",
  ).trim();
  if (!archivoUrl) {
    archivoUrl = extractMediaUrlBackend(msg);
  }
  return {
    id: msg.id || msg._id,
...
exports.getConversations = async (req, res) => {
  try {
    ...
    if (!rawConversations) {
      ...
      rawConversations = dedupeConversationsByPhonePerEstado(
        rawConversations.map((conv) => normalizeConversation(conv)),
      );
    }
...
exports.getMessages = async (req, res) => {
  try {
    ...
    if (global.chatModel?.getMensajes) {
      const mensajes = await global.chatModel.getMensajes(
        conversacionId,
        req.query.agentId || null,
        { ignoreAgentCheck },
      );
      return res.json(mensajes.map(normalizeMessage));
    }
...
exports.getMessageBufferStatus = async (req, res) => {
  try {
    const { conversacionId } = req.params;
    const messageBuffer = require("../services/messageBuffer.service");
    const pending = await messageBuffer.getBufferedMessages(conversacionId);
    ...
    if (global.chatModel?.getMensajes) {
      ...
      const conv = global.chatModel?.findConversationByAnyId
        ...
        : null;
      const { fetchMessagesForConversation } = require("../utils/messageQuery");
      const persisted = await fetchMessagesForConversation(
        db,
        conversacionId,
        conv,
      );
    ...
    res.json({
      ok: true,
      convId: String(conversacionId),
      redisPending: pending.length,
      mongoPersisted: persistedCount,
      pendingMessages: pending.map((msg) => ({
        id: msg.id,
        tempId: msg.tempId,
        mensaje: msg.mensaje || msg.text || msg.texto,
        tipo: msg.tipo,
        timestamp: msg.timestamp || msg.ts,
      })),
      redisKey: messageBuffer.bufferKey(conversacionId),
    });
  } catch (err) {
    res.status(500).json({
      ok: false,
      error: "Error al consultar buffer Redis",
      details: String(err?.message || err),
    });
  }
};
